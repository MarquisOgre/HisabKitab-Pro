    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <h1 className="text-lg md:text-xl font-bold text-foreground">Purchases</h1>
          {!isMobile && <p className="text-sm text-muted-foreground">Manage purchase invoices, payments and expenses</p>}
        </div>
        <Button className="gap-1.5 md:gap-2 shrink-0 h-8 md:h-10 text-xs md:text-sm px-2.5 md:px-4">
      ...
      <Tabs defaultValue="invoices" className="mobile-tabs">
        <TabsList className={isMobile ? "w-full grid grid-cols-3" : "grid w-full grid-cols-3 max-w-lg"}>
          ...
          <TabsTrigger value="payment-out" className="gap-1 md:gap-2 text-xs md:text-sm px-1.5 md:px-3">
            <ArrowUpRight className="w-3.5 h-3.5 hidden md:block" />
            Payment Out
          </TabsTrigger>
          <TabsTrigger value="expenses" className="gap-1 md:gap-2 text-xs md:text-sm px-1.5 md:px-3">
            <Wallet className="w-3.5 h-3.5 hidden md:block" />
            Expenses
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="invoices" className="space-y-4">
          <div className="grid grid-cols-3 gap-2 md:gap-4">
            <div className="stat-card p-3 md:p-4"><p className="text-[10px] md:text-xs text-muted-foreground uppercase">Total</p><p className="text-base md:text-xl font-bold text-foreground mt-1">{purchases.length}</p></div>
            <div className="stat-card p-3 md:p-4"><p className="text-[10px] md:text-xs text-muted-foreground uppercase">Amount</p><p className="text-base md:text-xl font-bold text-foreground mt-1">₹{purchases.reduce((s, p) => s + Number(p.total_amount || 0), 0).toLocaleString()}</p></div>
            <div className="stat-card p-3 md:p-4"><p className="text-[10px] md:text-xs text-muted-foreground uppercase">Unpaid</p><p className="text-base md:text-xl font-bold text-destructive mt-1">{purchases.filter(p => p.status === "unpaid").length}</p></div>
          </div>
              ...
              {purchases.map(p => (
                <div key={p.id} className="stat-card p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-sm text-foreground">{p.invoice_number}</span>
                    <Badge variant={p.status === "paid" ? "default" : "destructive"} className="text-[10px] px-2">{p.status}</Badge>
                  </div>
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>{p.parties?.name || "-"}</span>
                    <span>{p.invoice_date}</span>
                  </div>
                  <div className="text-right font-semibold text-foreground">₹{Number(p.total_amount || 0).toLocaleString()}</div>
                </div>
              ))}
              {purchases.length === 0 && <p className="text-center py-8 text-sm text-muted-foreground">No purchase invoices yet.</p>}
            </div>
          ) : (
        ...
        <TabsContent value="payment-out" className="space-y-4">
          {isMobile ? (
            <div className="space-y-2">
              ...
              {paymentsOut.length === 0 && <p className="text-center py-8 text-sm text-muted-foreground">No payments made yet.</p>}
            </div>
          ) : (
            <div className="stat-card p-0 overflow-hidden">
              <table className="w-full text-sm">
        ...
        <TabsContent value="expenses" className="space-y-4">
          {isMobile ? (
            <div className="space-y-2">
              {expenses.map(e => (
                <div key={e.id} className="stat-card p-3 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-sm text-foreground">{e.category}</span>
              ...
              {expenses.length === 0 && <p className="text-center py-8 text-sm text-muted-foreground">No expenses recorded yet.</p>}
            </div>
          ) : (
            <div className="stat-card p-0 overflow-hidden">
              <table className="w-full text-sm">
